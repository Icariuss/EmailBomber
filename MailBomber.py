...
class Email_Bomber:
    ...
    def __init__(self):
        try:
            ...
            self.mode = input('Enter BOMB mode (1,2,3,4) || 1:(1000) 2:(500) 3:(250) 4:(custom)')
            if self.mode > int(4) or int(self.mode) < int(1):
                print('ERROR: Invalid Option. GoodBye Friend.')
                sys.exit(1)
    ...
    def bomb(self):
        try:
            ...
            if self.mode == int(1):
                ...
                self.amount = int(bcolors.GREEN + input('Choose a custom amount <: '))
    ...
    def email(self):
        try:
            ...
            if self.server not in premade:
                default_port = False
                self.port = int (input(bcolors.GREEN + 'Enter part number <: '))
            ...
            self.fromAddr = str(input(bcolors.GREEN + 'Enter from adress <: '))
